...
public class AssortedMethods {
	...
	public static BinaryTreeNodeSimple randomBST(int N, int min, int max) {
		int d = randomIntInRange(min, max);
		BinaryTreeNodeSimple root = new BinaryTreeNodeSimple(d);
		for (int i = 1; i < N; i++) {
	...
	public static BinaryTreeNodeSimple createTreeFromArray(int[] array) {
		if (array.length > 0) {
			BinaryTreeNodeSimple root = new BinaryTreeNodeSimple(array[0]);
			java.util.Queue<BinaryTreeNodeSimple> queue = new java.util.LinkedList<BinaryTreeNodeSimple>();
			queue.add(root);
			boolean done = false;
			int i = 1;
			while (!done) {
				BinaryTreeNodeSimple r = (BinaryTreeNodeSimple) queue.element();
				if (r.left == null) {
					r.left = new BinaryTreeNodeSimple(array[i]);
					i++;
					queue.add(r.left);
				} else if (r.right == null) {
					r.right = new BinaryTreeNodeSimple(array[i]);
					i++;
					queue.add(r.right);
				} else {
					queue.remove();
